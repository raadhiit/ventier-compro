        ...
        @if($categories->isNotEmpty())
            <div class="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                @foreach($categories as $category)
                    <article wire:key="home-product-category-{{ $category->id }}" class="group h-full min-h-96 overflow-hidden rounded-[2rem] border border-border-sand bg-white transition duration-500 hover:-translate-y-1 hover:border-champagne/50 hover:shadow-xl hover:shadow-black/5 motion-reduce:transform-none motion-reduce:transition-none" data-reveal>
                        <a href="{{ route('products.index', ['category' => $category->slug]) }}" wire:navigate class="flex h-full flex-col p-7">
                            <div class="flex size-12 shrink-0 items-center justify-center rounded-2xl border border-champagne/25 bg-champagne/10 text-champagne-dark">
